<?php
session_start();
if (isset($_SESSION['lastpath']) && isset($_SESSION['login'])) {
	header('Location: '. $_SESSION['lastpath']);
	unset($_SESSION['lastpath']);
}
...
if (isset($_GET['num_files'])) {
	$_SESSION['num_files'] = $_GET['num_files'];

}

#if (!isset($_SESSION['num_files'])) {
	?>
<form action>
<label>Select Number of files to upload</label>
		<select  name="num_files" >
...
?>
	<form action method="post" enctype="multipart/form-data">
		<label>uploading to <span style="color: #f00"><?= $dir ?> </span></label>
		<fieldset>
		<label>select files</label>
		<?php
		foreach (range(1, $_SESSION['num_files']) as $i ) {
			?>
 <input type="file" name="file[]" value="Pasirinkit faila">
			<?php
			# code...
		}
		?>
		...
		<input type="submit" name="Ikelti faila">
...
if (isset($_FILES) && !empty($_FILES)) {
	...
	move_uploaded_file($_FILES['file']['tmp_name'][$i], $new_path);

	?>
	<br>
	<?php
	$uploaded_link = $dir;	
	$uploaded_link = str_replace('/home/me/public_html/', 'http://127.0.0.1/~me/',  $uploaded_link);
	if ($_FILES['file']['error'][$i] == 0) {
	?>
	 <strong>FILE ( <?= $i ?> )<strong> <span style="color: #0f0">File uploaded to <?= $dir ?></span> <a href="<?= $uploaded_link . '/' . $base_name ?>">Click here to open it</a>
	<?php
}
